...
namespace RedJasmine\FilamentSupport\Forms\Components;
...
class SelectTree extends \CodeWithDennis\FilamentSelectTree\SelectTree
{
    ...
    public function getQuery() : ?Builder
    {
        ...
        if ($this->withTranslation) {
            ...
            if ($query->getModel()->isRelation('translations')) {
                // TODO
    ...
    public function withTranslation(bool $withTranslation = true) : static
    {
        ...
        $this->getOptionLabelUsing(function ($record) {

            if ($record->isRelation('translations')) {
                return $record->translate() ?
                    $record->translate()->{$this->getTitleAttribute()}
                    : $record->{$this->getTitleAttribute()};

            } else {
                return $record->{$this->getTitleAttribute()};
            }
